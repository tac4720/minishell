...
#include "minishell.h"// void create_infile_redir_node(t_cmd *cmd, t_token *token, int command_flag, t_context *context)
// {
// 	cmd->infile_redir = ft_calloc(1, sizeof(t_infile_redir));
// 	if (cmd->infile_redir == NULL)
// 		error_in_parse(context);
// 	cmd->infile_redir->filename = ft_strdup(token->str);
// 	cmd->infile_redir->next = NULL;
// 	cmd->infile_redir->redirection_flag |= command_flag;
// }
void create_infile_redir_node(t_token **token_list, t_cmd *cmd, int command_flag, t_context *context)
{
	cmd->infile_redir = ft_calloc(1, sizeof(t_infile_redir));
	if (cmd->infile_redir == NULL)
		error_in_parse(context);
...
void append_infile_redir_node(t_token **token_list, t_cmd *cmd, int command_flag, t_context *context)
{
	t_infile_redir *tmp = cmd->infile_redir;
...
void create_outfile_redir_node(t_token **token_list, t_cmd *cmd, int command_flag, t_context *context)
{
...
void append_outfile_redir_node(t_token **token_list, t_cmd *cmd, int command_flag, t_context *context)
{
	t_outfile_redir *tmp = cmd->outfile_redir;
